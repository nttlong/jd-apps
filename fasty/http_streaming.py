...
async def __send_bytes_range_requests__(
    ...
    """
    file_obj= await fasty.config.storage.file.open_stream(file_obj)
    file_obj.seek(start)
    data = [1]
    pos = file_obj.tell()
    while len(data) > 0:
        read_size = min(chunk_size, end + 1 - pos)
        data = await fasty.config.storage.file.read(file_obj,read_size)
        yield data
...
async def streaming(fsg,request,content_type,streaming_buffering=1024 *  8*4):
    ...
    :return:
    """
    file_size =await fasty.config.storage.file.get_len_of_file_stream(fsg)
