import ReCompact.dbm.DbObjects.compilers
import ReCompact.dbm.DbObjects.expressions
...
def to_mongobd(expr,*args,**kwargs):
    ...
    tree=ReCompact.dbm.DbObjects.compilers.compile_expression(_expr)
...
def to_mongodb_expr(fx,params,forSelect=False,forNot=False,prefix=None):
    import re
    if isinstance(fx,ReCompact.dbm.DbObjects.expressions.StringLiteralExpression):
        return fx.value
    if isinstance(fx,ReCompact.dbm.DbObjects.expressions.NumericLiteralExpression):
        return fx.value
    if isinstance(fx,ReCompact.dbm.DbObjects.expressions.UnaryExpression):
        if fx.type=="-":
    ...
    if isinstance(fx,ReCompact.dbm.DbObjects.expressions.IdentifierExpression):
        if prefix!=None:
    ...
    if isinstance(fx,ReCompact.dbm.DbObjects.expressions.BinaryExpression):
        ...
        if fx.operator=="==":
            if type(left) == str:
                if type(right) == str and (not forSelect):
        ...
        if fx.type==ReCompact.dbm.DbObjects.expressions.expression_types.LOGICAL_EXP:
            return {
