from ReCompact.dbm.DbObjects.expressions import rec_process,BinaryExpression,char_codes,expression_types,binary_ops
from ReCompact.dbm.DbObjects.expressions import unary_ops,literals,isDecimalDigit,isIdentifierStart,isIdentifierPart
from ReCompact.dbm.DbObjects.expressions import NumericLiteralExpression,StringLiteralExpression,ArrayExpression
from ReCompact.dbm.DbObjects.expressions import UnaryExpression,MemberExpression,CallExpression,LiteralExpression,ThisExpression
from ReCompact.dbm.DbObjects.expressions import IdentifierExpression,ConditionalExpression
from ReCompact.dbm.DbObjects.expressions import CompoundExpression,BinaryOperatorInfo
...
def gobbleIdentifier(rec):
    ...
    if literals.get(identifier,None):
...
def gobbleToken(rec):
    ...
    if isDecimalDigit(ch) or ch ==char_codes.PERIOD_CODE: #Char code 46 is a dot `.` which can start off a numeric literal
        ...
        while tc_len>0:
            ...
            """
            if unary_ops.get(to_check,None) and\
                    (
